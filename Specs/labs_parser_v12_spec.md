...
## API Endpoints (v12 additions to v7 API)

These endpoints power the Agent Run and Protocol screens in the frontend:

| Method | Path | Purpose |
|--------|------|---------|
| `POST` | `/agent/run` | Start an agent run with a goal string. Returns `run_id`, streams progress via SSE. |
| `GET` | `/agent/runs` | List past agent runs (for Dashboard's "past runs" section). |
| `GET` | `/agent/runs/{run_id}` | Get a completed run's summary, steps, stats, findings. |
| `GET` | `/agent/runs/{run_id}/steps` | Get the step-by-step timeline (for replay). |
| `GET` | `/protocols/{run_id}` | Get the protocol generated by a run. Returns `Protocol` model. |
| `POST` | `/protocols/{run_id}/start` | Mark a protocol as "started" (user committed to it). |
...
class Intervention(BaseModel):
    ...
    group: Literal["nutrition", "training", "supplement", "monitoring"]  # frontend groups by this
    dose: str | None
    timing: str | None
    expected_effect: str
    ...
    warn: str | None = None            # single-line warning for frontend card (e.g., "flush, LFTs at 8 wk")
    highlight: bool = False            # frontend renders accent card when True
...
class Protocol(BaseModel):
    ...
    targets: list[str]                 # target chips: "HDL → 55+", "Lp(a) → <75"
    retest_date: str | None            # ISO date for next lab draw
